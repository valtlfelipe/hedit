      ...
      <transition name="slide-up">
        <div v-if="show" class="w-full max-w-md p-6 bg-white rounded-lg shadow-xl dark:bg-zinc-800">
          <div class="flex items-center justify-between">
            ...
            <button class="p-1 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-gray-200 dark:hover:bg-zinc-700" @click="emit('close')">
              <X class="w-5 h-5" />
            </button>
          </div>
          ...
          <div class="mt-6">
            ...
            <div v-if="!personalUseOnly" class="mt-6">
              ...
              />
              <div v-if="error" class="mt-2 text-sm text-red-600 dark:text-red-400">{{ errorText }}</div>
            </div>
          </div>

          <div class="mt-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <a href="#" class="text-sm text-purple-600 hover:text-purple-800 hover:underline dark:hover:text-purple-400" @click.prevent="openPurchasePage()">
              Purchase License
            </a>
